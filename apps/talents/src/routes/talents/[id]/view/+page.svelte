...
import TaskList from "$lib/components/tasks/TaskList.svelte";
    ...
    $effect(() => {
        readTalent(id).then((t) => {
            if (t?.contact) {
                breadcrumbState.set({
                    feature: "talents",
                    current: `View: ${t.contact.displayName}`,
                });
            }
        }).catch(() => {});
    });
...
<div class="max-w-4xl mx-auto px-4 py-8">
    <div class="mb-6 flex items-center justify-between">
        ...
        <a href="/talents/{id}">
            <Button variant="outline" size="sm">
                <Edit size={16} class="mr-2" /> Edit Talent
            </Button>
        </a>
    </div>
    ...
    <svelte:boundary>
        ...
        <div class={[$effect.pending() && "opacity-50 pointer-events-none"]}>
            {#await readTalent(id) then talent}
                {#if talent && talent.contact}
                    {@const contact = talent.contact as any}
                    <div
                            ...
                            <div class="space-y-8">
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm font-bold uppercase text-indigo-600">
                                                    {talent.status}
                                                </p>
                                            </div>
                                        </div>
                                ...
                                <section>
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        {/each}
                                        {#if !contact.locationAssociations?.length}
        ...
        {#snippet failed(error: unknown)}
            <ErrorSection headline="Error" message={error instanceof Error ? error.message : "An error occurred"} />
        {/snippet}
    </svelte:boundary>
</div>
