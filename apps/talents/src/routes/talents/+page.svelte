<script lang="ts">
    ...
    const talentsQuery = $derived(listTalents() as any);
    let searchQuery = $state("");
    let selectedIds = $state<string[]>([]);

    let filteredTalents = $derived(
        talentsQuery.data?.filter((t: any) => {
    ...
    function toggleSelectAll() {
        if (selectedIds.length === filteredTalents.length) {
            ...
            selectedIds = filteredTalents.map((t: any) => t.id);
        }
    }
    ...
    const stats = $derived({
        total: talentsQuery.data?.length || 0,
        applicants: talentsQuery.data?.filter((t: any) => t.status === 'applicant').length || 0,
        active: talentsQuery.data?.filter((t: any) => t.status === 'active').length || 0,
    });
</script>
