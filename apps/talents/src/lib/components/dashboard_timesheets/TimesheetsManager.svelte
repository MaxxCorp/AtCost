            ...
            <div class="lg:col-span-2 space-y-8">
                        ...
                        <div class="flex flex-col items-center justify-center space-y-8 relative z-10 text-center">
                            ...
                            <div class="space-y-1">
                                <h1 class="text-4xl font-black text-gray-901 tracking-tighter leading-tight uppercase italic">
                                    {isActive ? m.live_stream_active() : m.session_standby()}
                                </h1>
                                <div class="mt-2 text-center flex justify-center">
                                    {#if isActive && status.activeEntry}
                                        <div class="inline-flex items-center gap-2 bg-emerald-50 text-emerald-700 px-4 py-1.5 rounded-full border border-emerald-100 shadow-sm">
                                            ...
                                            <span class="text-[10px] font-black uppercase tracking-widest">{m.entry_confirmed_at()} {format(new Date(status.activeEntry.startTime), 'HH:mm')}</span>
                                        </div>
                                    {:else}
                                        <p class="text-gray-400 text-xs font-bold tracking-[0.2em] italic uppercase">{m.internal_clock_synchronized()}</p>
                                    {/if}
                                </div>
                            </div>
                ...
                <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
                    <div class="px-8 py-6 border-b border-gray-50 flex justify-between items-center bg-gray-50/30">
                        <h2 class="font-black text-gray-901 text-lg tracking-widest uppercase italic border-l-4 border-indigo-600 pl-4 leading-none">{m.activity_log()}</h2>
                        <button 
                            type="button"
                    ...
                    <div class="divide-y divide-gray-50">
                                    ...
                                    <div class="flex items-center gap-6">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-[10px] text-gray-400 font-bold tabular-nums tracking-widest uppercase">
                                                ...
                                                {:else}
                                                    <span class="text-emerald-500 font-black animate-pulse">{m.live()}</span>
                                                {/if}
                                            </div>
                                        </div>
                                    </div>
                                    <div class="flex flex-col items-end gap-1.5">
                                        {#if entry.endTime}
                                            <div class="text-[18px] font-black text-gray-901 tabular-nums tracking-tighter italic">
                                                {(differenceInMinutes(new Date(entry.endTime), new Date(entry.startTime)) / 60).toFixed(1)}<span class="text-[10px] text-gray-400 ml-0.5 uppercase">{m.hrs()}</span>
                                            </div>
                                        {/if}
                        ...
                        {:else}
                            <div class="p-12 text-center">
                                <p class="text-gray-300 font-black uppercase tracking-widest italic text-[10px]">{m.no_historical_data()}</p>
                            </div>
                        {/if}
                    </div>
                </div>
