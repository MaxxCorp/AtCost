...
export type TalentFeature = 'users' | 'talents' | 'locations' | 'timesheets' | 'my-profile' | 'time-off' | 'shiftplans' | 'talent-groups';
...
export function hasAccess(user: UserWithRolesAndClaims, feature: TalentFeature): boolean {
    ...
    if (feature === 'talent-groups') return sharedHasAccess(user, 'talents');
